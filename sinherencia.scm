...
(define the-grammar
  '((program ((arbno table-decl) (arbno class-decl) expression) a-program)
...
;^;;;;;;;;;;;;;;; new productions for oop ;;;;;;;;;;;;;;;;
    ...
    (expression
     ("table." identifier "." "insert"
               "(" (separated-list expression ",") ")" ) table-insert-exp)
    )
...
(define eval-program 
  (lambda (pgm)
    (cases program pgm
      (a-program (t-decls c-decls exp)
        (elaborate-class-decls! c-decls)
        (elaborate-table-decls t-decls);\new1
...
;&
      ...
      (for-exp (id rand conditional incr body)
               (let ((new-rand (make-vector 1 (eval-rand rand env))))
                 (let ((new-env  (extend-env-refs (list id) new-rand env)))
                   ;(let ((x 0))
                     
                     (local [(define for-aux
                               (lambda (conditional incr body env return)
                                 (if (eval-expression conditional env)
                                     (begin (set! return (eval-expression body env))
                                            (eval-expression incr env)
                                            (for-aux conditional incr body env return))
                                     return)))]
                       (for-aux conditional incr body new-env 0))
                   ;  )
                   )
                 )
               )
      
    ;               (let ((new-env  (extend-env (list id) (list (eval-rand rand)) env)))
;                 (do ((id (eval-rand rand) ()))
    ;                  ((equal? i 10))
;                   (display (+ i 1)))
...
;^;;;;;;;;;;;;;;; begin new cases for chap 5 ;;;;;;;;;;;;;;;;
      ...
      (table-insert-exp (table-name elements)
...
(define table-decl->encabezados
  (lambda (t-decl)));;; you are here ! at 29/11/2017 10:44 am.
...
(define elaborate-table-decls
  (lambda (t_decls)
    (set! table-env t_decls)))
